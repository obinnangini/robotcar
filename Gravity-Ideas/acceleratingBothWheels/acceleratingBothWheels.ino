...
ISR(TIMER1_OVF_vect)          // timer compare interrupt service routine
{
  ...
  motors.setSpeeds(
    round(deadReckoningNextMotorSpeedA*calibratedMotorPwm(1.0, 1.0, deadReckoningNextMotorSpeedA)),
    round(deadReckoningNextMotorSpeedB*calibratedMotorPwm(0.5, 0.5, deadReckoningNextMotorSpeedB)));
...
double g=100.0;
double vi=100.0;
int verticalSpeedAfterBeingThrownUp(double milliseconds, boolean falling) {
  /* displacement = ax+bx^2 */
  /* speed = a + 2bx */
  if(!falling) {
    return round(vi-milliseconds/1000.0*g);
  } else {
    return round(milliseconds/1000.0*g);
  }
}
...
void loop() {
 ...
 while ( !falling || speed < vi) { 
  /* Bouncing ball */
  int speed = verticalSpeedAfterBeingThrownUp(totalMilliseconds, falling);
  if (speed <= 0) {
    falling = true;
  /*  vi = 0.9*vi; */
    totalMilliseconds=0;
